...
;; sheep is its own plural: we use "a-sheep" as the singular.
breed [gallinas gallina]
breed [lobos lobo]
lobos-own[comido]

to configuracion-inicial
  clear-all
  ask patches[set clima random 41] ;;da valores a las parcelas de clima "ask patches[set plabel clima]"
  ask patches
  [
    ifelse clima <= 15
    [ set pcolor sky ]
    [
   ifelse clima >= 38
      [ ask patches in-radius 3 [set pcolor orange]
        set pcolor red]
      [set pcolor orange]
    ]
  ...
  set-default-shape gallinas "bird"
  create-gallinas numero-inicial-de-gallinas [ ;; create the sheep
  ...
  create-lobos numero-inicial-de-lobos [ ;; create the wolves
    ;; then initialize their variables
    set label comido
    set color black
    set size 1.5  ;; easier to see
...
to go
  ...
  ask lobos [
    ...
    if (pcolor = 95)
    [
      set label round comido
      catch-sheep]
  ]
end

to catch-sheep  ;; wolf procedure
  let prey one-of (gallinas in-radius 3)
